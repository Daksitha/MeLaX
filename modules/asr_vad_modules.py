...
class WhisperTranscriptionThread(QThread):
    ...
    def __init__(self, model="medium", device="cpu", language="en", energy_threshold=1000, record_timeout=2.0, parent=None, logger=None, microphone_name=None):
    ...
    def run(self):
        try:
            ...
            if self.microphone_name:
                ...
                try:
                    mic_index = mic_names.index(self.microphone_name)
                except ValueError:
                    self.logger.log_error("Selected microphone not found. Using default microphone.")
                    mic_index = None
            else:
                mic_index = None

            # Create the microphone source with the determined device_index if available
...
class GoogleASRTranscriptionThread(QThread):
    ...
    def __init__(self, api_key, endpoint, language, energy_threshold=1000, record_timeout=2.0, parent=None, logger=None,microphone_name=None ):
        super().__init__(parent)
